...
The v3 plugin (`permixPlugin`, `PermixDefinition`, session-driven `createPermix<Definition, Session>`) was **redesigned**. v4 generates Permix from your Better Auth **access control statement** and maps roles with **`roleToRules()`**:

[Better Auth integration](/docs/integrations/better-auth)

If you used the v3 HTTP plugin and session helpers, plan a full pass over server and client setup using the new guide.
...
| `permix/better-auth` | Access-control-driven Permix |
...
### Revisit Better Auth (if used)

Follow the new [Better Auth](/docs/integrations/better-auth) guide.
