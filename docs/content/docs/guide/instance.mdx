...
List of action names for the entity. Use a tuple of strings for simple actions, or action spec objects (`{ name, type?, typeRequired? }`) when an action needs a typed entity.
...
By default, when an action declares a `type`, the data argument in `check` is optional. Set `typeRequired: true` to require data for that action.
...
When `typeRequired` is `true`, TypeScript requires the data argument for that action. Use this when permission logic always depends on entity data.
