...
By default, a `type` on an action makes the data argument optional in `check`. Set `typeRequired: true` to require it.
...
const permix = createPermix<{
  post: [{ name: 'update', type: Post, typeRequired: true }]
