...
Invalid paths cause a TypeScript error at compile time. At runtime, checking an undefined path throws `PermixRuleNotDefinedError`. In React, the `usePermix` hook catches check errors and returns `false` instead (including undefined paths).
...
| `PermixRuleNotDefinedError` | `check()` targets a path that is not present in the current rules object. |
