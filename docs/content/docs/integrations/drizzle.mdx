---
title: Drizzle
description: Auto-generate a Permix statement and CRUD rules from your Drizzle schema
---
...
| `permix/drizzle/v0`      | **v0** (`>=0.30 <1`)       | Detects tables only via `is(value, Table)`.                                        |
...
<Callout>
Before getting started with the Drizzle integration, make sure you've completed the initial setup steps in the [Quick Start](/docs/quick-start) guide.
</Callout>
...
import { integer, pgTable, serial, text } from 'drizzle-orm/pg-core'
import { createPermix } from 'permix/drizzle/v0'
...
## Generating Baseline Rules

The Drizzle integration adds a `generateRules` helper that produces a fully-populated rules object covering every table in the schema. It accepts three shapes:

```ts
permix.setup(permix.generateRules(true))

permix.setup(permix.generateRules({ read: true }))

permix.setup(permix.generateRules((table, action) => {
  if (table === 'users')
...
Combine it with `template` to define reusable presets:

```ts
const adminTemplate = permix.template(permix.generateRules(true))
const guestTemplate = permix.template(permix.generateRules({ read: true }))
