...
You `setupMiddleware()` the rules once per request and `get()` them anywhere on the server — `beforeLoad`, loaders, server routes, and server functions — without threading the instance through props. The client side reuses the React integration (`permix/react`) via `PermixProvider` + `PermixHydrate`.
