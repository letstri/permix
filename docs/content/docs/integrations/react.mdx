...
For SSR applications, use `PermixHydrate` to restore dehydrated server state on the client. After hydration, call `setup()` again to restore function-based rules:
