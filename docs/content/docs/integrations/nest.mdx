...
<Callout>
  `permix.guard(...)` only sets up the per-request instance — `@Check` does the
  enforcing. If more than one guard from the same factory applies to a request
  (a global `APP_GUARD` plus a per-controller one, say), the last one to run
  wins: its rules are what `@Check` enforces and what your handler reads back
  through `permix.getOrThrow(req)`.
</Callout>
