...
<Callout>
  ...
  contexts (RPC, WebSockets) are skipped, so the guard is safe to register
  globally in a hybrid app.
</Callout>

Because the request shape depends on the HTTP adapter, `req` is untyped by default. Pass your adapter's request type as the second type argument to get it typed everywhere — in the guard callback, `onForbidden`, `get`, `getOrThrow` and `getRules`:

```ts
import type { Request } from 'express'

export const permix = createPermix<Definition, Request>()
```
