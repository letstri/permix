...
- **Server checks** — loaders and server functions call `permix.get(context)` / `checkMiddleware()`.
...
| `src/server/posts.ts` | Server functions with loader checks and `checkMiddleware` |
